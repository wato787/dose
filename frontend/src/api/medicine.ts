...
import type { FrequencyType, Medicine, NewCustomItem, NewMedicine } from "@/types/domain";
...
export const getMedicines = async (params?: {
  ...
  const response = await get<{ data: { medicines: Medicine[]; count: number } }>(
          ...
          ? {
              ...
              startDate:
                data.schedule.startDate instanceof Date
                  ? data.schedule.startDate.toISOString()
...
export const createCustomLog = async (
  ...
  const response = await post<{
    data: { customLogId: number; customItemId: number; recordDate: Date; value: string };
 ...
 */
export const updateCustomLog = async (
  medicineId: number,
  customLogId: number,
  data: {
    recordDate?: Date;
